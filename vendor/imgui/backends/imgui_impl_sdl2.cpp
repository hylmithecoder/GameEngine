...
//  ImGuiPlatformIO interface. 2025-04-09: [Docking] Revert update monitors and
//  work areas information every frame. Only do it on Windows. (#8415, #8558)
//  2025-04-09: Don't attempt to call SDL_CaptureMouse() on drivers where we
//  don't call SDL_GetGlobalMouseState(). (#8561) 2025-03-21: Fill gamepad
//  inputs and set ImGuiBackendFlags_HasGamepad regardless of
...
bool ImGui_ImplSDL2_ProcessEvent(const SDL_Event *event) {
  ...
  case SDL_KEYUP: {
    ...
    if (viewport == nullptr)
      return false;
    // IMGUI_DEBUG_LOG("SDL_KEY%s : key=0x%08X ('%s'), scancode=%d ('%s'),
    // mod=%X, windowID=%d, viewport=%08X\n",
    //     (event->type == SDL_KEYDOWN) ? "DOWN" : "UP  ",
    //     event->key.keysym.sym, SDL_GetKeyName(event->key.keysym.sym),
    //     event->key.keysym.scancode,
    //     SDL_GetScancodeName(event->key.keysym.scancode),
    //     event->key.keysym.mod, event->key.windowID, viewport ? viewport->ID :
    //     0);
  ...
  case SDL_WINDOWEVENT: {
    ...
    // IMGUI_DEBUG_LOG("SDL_WINDOWEVENT_FOCUS_GAINED: windowId %d, viewport:
    // %08X\n", event->window.windowID, viewport ? viewport->ID : 0); } if
    // (window_event == SDL_WINDOWEVENT_FOCUS_LOST) {
    // IMGUI_DEBUG_LOG("SDL_WINDOWEVENT_FOCUS_LOST: windowId %d, viewport:
...
void ImGui_ImplSDL2_NewFrame() {
  ...
  // Update monitors
#ifdef WIN32
  bd->WantUpdateMonitors = true; // Keep polling under Windows to handle changes
                                 // of work area when resizing task-bar (#8415)
#endif
  ...
  // rigorous, but testing for payload reduces noise and potential side-effects.
...
static void ImGui_ImplSDL2_ShowWindow(ImGuiViewport *viewport) {
  ...
  SDL_SetHint(SDL_HINT_WINDOW_NO_ACTIVATION_WHEN_SHOWN,
              (viewport->Flags & ImGuiViewportFlags_NoFocusOnAppearing) ? "1"
                                                                        : "0");
#elif defined(_WIN32)
