...
public:
  ...
  void createGraphicsPipeline(const string &vertShaderPath,
  ...
  VkCommandBuffer offscreenCmdBuffer =
      VK_NULL_HANDLE; // Offscreen command pool + semaphore
  ...
  void showCurrentCameraPosition() {
    ImGui::Text("Camera Position: (%.2f, %.2f, %.2f)",
                camera.matrices.view[3][0], camera.matrices.view[3][1],
  ...
  void DrawImage();
  uint32_t FindMemoryType(VkMemoryRequirements memRequirements,
  ...
  // properties, VkImage& image, VkDeviceMemory& imageMemory); void
  // TransitionImageLayout(VkImage image, VkFormat format, VkImageLayout
  ...
  void EndSingleTimeCommands(VkCommandBuffer commandBuffer) {
    ...
    fenceCI.flags = 0;
    VkFence fence;
    VK_CHECK_RESULT(vkCreateFence(g_Device, &fenceCI, nullptr, &fence));

    vkQueueSubmit(g_Queue, 1, &submitInfo, VK_NULL_HANDLE);
    // check_vk_result(vkWaitForFences(g_Device, 1, &fence, VK_TRUE,
    // 100000000000)); vkDestroyFence(g_Device, fence, nullptr);
  ...
  static void check_vk_result(VkResult err) {
    ...
    case VK_SUBOPTIMAL_KHR:
      // Log("Swap chain out of date or suboptimal - rebuilding...",
      // LogLevel::WARNING); g_SwapChainRebuild = true;
      break;
    default:
      if (err < 0) {
  ...
  void helperInitImage() {
    ...
    imageHandler.setCurrentDeviceAndPhysic(g_Device, g_PhysicalDevice, g_Queue,
                                           g_QueueFamily, offscreenCommandPool,
                                           memoryProperties);
    textureHandler.setCurrentDeviceAndPhysic(
        g_Device, g_PhysicalDevice, g_Queue, offscreenCommandPool,
        memoryProperties, renderPass, deviceProps, deviceFeatures,
        g_DescriptorPool, framebuffers);
  }
...
private:
  ...
  VkGraphicsPipelineCreateInfo createPipeLineInfo(
      VkPipelineShaderStageCreateInfo *currentShaderStage,
  ...
  createShaderStageInfo(VkShaderModule shaderModule) {
