...
public:
  ...
  void draw() {
    ...
    // Set uniform values
    glUniform2f(glGetUniformLocation(shaderProgram, "uViewport"), viewportWidth,
                viewportHeight);
    glUniform2f(glGetUniformLocation(shaderProgram, "uPan"), pan.x, pan.y);
    glUniform1f(glGetUniformLocation(shaderProgram, "uZoom"), zoom);
    glUniform3f(glGetUniformLocation(shaderProgram, "uGridColor"), gridColor.x,
                gridColor.y, gridColor.z);
    glUniform3f(glGetUniformLocation(shaderProgram, "uBgColor"), bgColor.x,
                bgColor.y, bgColor.z);
    ...
    if (ImGui::IsItemHovered()) {
      ...
      if (ImGui::GetIO().MouseWheel != 0.0f) {
        ...
        ImVec2 mousePosInViewport =
            ImVec2(mousePos.x - viewportPos.x, mousePos.y - viewportPos.y);
      ...
      ImVec2 mousePosInViewport =
          ImVec2(mousePos.x - viewportPos.x, mousePos.y - viewportPos.y);
      ImVec2 worldPos =
          ImVec2((mousePosInViewport.x - viewportSize.x * 0.5f) / zoom + pan.x,
                 (mousePosInViewport.y - viewportSize.y * 0.5f) / zoom + pan.y);
    ...
    // Status bar
    ImGui::Text("Zoom: %.2fx | Pan: (%.1f, %.1f) | Grid Size: %.1f", zoom,
                pan.x, pan.y, gridSize);
