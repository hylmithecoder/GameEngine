  ...
  {                                                                            \
    ...
    if (res < 0) {                                                             \
      LOGE("Fatal : VkResult is \" %s \" in %s at line %d",                    \
           vkhandler::tools::errorString(res).c_str(), __FILE__, __LINE__);    \
  ...
  {                                                                            \
    ...
    if (res < 0) {                                                             \
      std::cout << "Fatal : VkResult is \""                                    \
                << vkhandler::tools::errorString(res) << "\" in " << __FILE__  \
                << " at line " << __LINE__ << "\n";                            \
      assert(res == VK_SUCCESS);                                               \
    }                                                                          \
  }
...
namespace vkhandler {
namespace tools {
/** @brief Setting this path chnanges the place where the samples looks for
 * assets and shaders */
