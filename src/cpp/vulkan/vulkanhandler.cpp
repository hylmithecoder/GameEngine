...
VkDescriptorSet VulkanHandler::LoadImage(const char *filename) {
  ...
  DEBUG_LOG("Texture size: %llu", (unsigned long long)imageSize);
  ...
  if (vkCreateImage(currentDevice, &imageInfo, nullptr, &textureImage) !=
      VK_SUCCESS)
  ...
  VkMemoryAllocateInfo allocInfo =
      createMemoryAllocateInfo(memRequirements, currentPhysicalDevice);

  VkDeviceMemory textureMemory;
  if (vkAllocateMemory(currentDevice, &allocInfo, nullptr, &textureMemory) !=
      VK_SUCCESS)
  ...
  vkGetImageSubresourceLayout(currentDevice, textureImage, &subresource,
                              &layout);
  ...
  if (vkCreateImageView(currentDevice, &viewInfo, nullptr, &textureImageView) !=
      VK_SUCCESS)
  ...
  if (vkCreateSampler(currentDevice, &samplerInfo, nullptr, &textureSampler) !=
      VK_SUCCESS)
  ...
  Log("TextureSampler loaded: " +
      to_string(reinterpret_cast<uintptr_t>(textureSampler)));
  Log("TextureImageView loaded: " +
      to_string(reinterpret_cast<uintptr_t>(textureImageView)));
  // Tambahin ke ImGui
...
void VulkanHandler::setCurrentDeviceAndPhysic(
    VkDevice device, VkPhysicalDevice physicalDevice, VkQueue graphicsQueue,
    uint32_t currentGraphicQueue, VkCommandPool commandPool,
  ...
  // poolInfo.queueFamilyIndex = &currentGraphicsQueue; // atau
  // graphicsQueueFamily
}
...
bool VulkanHandler::OpenFileVideo(const char *filePath) {
  ...
  if (avformat_find_stream_info(formatContext, NULL) < 0) {
    ...
    cout << "[MainWindow] Video stream information found " << formatContext
         << endl;
  }
  ...
  if (avcodec_parameters_to_context(
          codecContext, formatContext->streams[videoStream]->codecpar) < 0) {
  ...
  av_image_alloc(lastGoodFrameRGB->data, lastGoodFrameRGB->linesize, width,
                 height, AV_PIX_FMT_RGB24, 1);
  hasValidFrame = false;
...
bool VulkanHandler::createVideoTexture() {
  ...
  if (vkCreateBuffer(currentDevice, &bufferInfo, nullptr, &stagingBuffer) !=
      VK_SUCCESS) {
  ...
  if (vkAllocateMemory(currentDevice, &allocInfo, nullptr,
  ...
  if (vkCreateImage(currentDevice, &imageInfo, nullptr, &videoImage) !=
      VK_SUCCESS) {
  ...
  if (vkAllocateMemory(currentDevice, &allocInfo, nullptr, &videoImageMemory) !=
      VK_SUCCESS) {
  ...
  if (vkCreateImageView(currentDevice, &viewInfo, nullptr, &videoImageView) !=
      VK_SUCCESS) {
  ...
  if (vkCreateSampler(currentDevice, &samplerInfo, nullptr, &videoSampler) !=
      VK_SUCCESS) {
  ...
  videoDescriptorSet = ImGui_ImplVulkan_AddTexture(
      videoSampler, videoImageView, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
...
bool VulkanHandler::openAudio() {
  ...
  av_opt_set_int(swrContext, "in_sample_rate", audioCodecContext->sample_rate,
                 0);
  av_opt_set_int(swrContext, "out_sample_rate", 44100,
                 0); // Fixed output rate for SDL3
  ...
  cout << "Input: " << srcSpec.freq << "Hz, " << (int)srcSpec.channels
       << " channels" << endl;
  cout << "Output: " << dstSpec.freq << "Hz, " << (int)dstSpec.channels
       << " channels" << endl;
...
void VulkanHandler::updateAudio() {
  ...
  while (!packetProcessed &&
         (readResult = av_read_frame(formatContext, pkt)) >= 0) {
    if (pkt->stream_index ==
        audioStreamIndex) { // Assuming you have audioStreamIndex
...
void VulkanHandler::updateBothVideoAndAudio24fps() {
  ...
  // if (hasAudio && SDL_GetQueuedAudioSize(audioDeviceID) <
      ...
      if (sendResult >= 0) {
        ...
        if (receiveResult >= 0) {
          ...
          sws_scale(swsContext, (uint8_t const *const *)frame->data,
                    frame->linesize, 0, height, frameRGB->data,
                    frameRGB->linesize);
          ...
          if (frame->pts != AV_NOPTS_VALUE) {
            AVRational timeBase =
                formatContext->streams[videoStream]->time_base;
            currentTime = frame->pts * av_q2d(timeBase);
          }
          ...
          if (allocResult >= 0) {
            ...
            if (convertedSamples >= 0) {
              ...
              if (outBufSize > 0) {
                ...
                if (SDL_GetAudioStreamQueued(audioStream) >=
...
VulkanHandler::loadShader(const char *fileName, VkShaderStageFlagBits stage) {
  ...
  try {
    ...
    shaderStage.module = vkhandler::tools::loadShader(
