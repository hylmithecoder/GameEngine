    ...
    HandleBackground(pos, size); // panggil di sini!
        // ::Log("Asset Folder: " + assetFolder.fullPath + "\nChildren: "+to_string(assetFolder.children.size()), Debug::LogLevel::INFO);
            if (firstOpenProject) {
                ...
                // ::Log("If You See it this is work in method RenderExplorerWindow", Debug::LogLevel::SUCCESS);
...
void MainWindow::HandleBackground(const ImVec2& windowPos, const ImVec2& windowSize) {
    ...
    g = assets.g;
    b = assets.b;
    // ::Log("Dominant Color: " + to_string(r) + ", " + to_string(g) + ", " + to_string(b), Debug::LogLevel::INFO);
