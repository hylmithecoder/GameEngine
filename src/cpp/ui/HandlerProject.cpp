...
void HandlerProject::OpenProject(const char* folderPath) {
    ...
    if (configFile.good()) {
        ::Log("Loading project configuration: " + string(readConfigPath), Debug::LogLevel::SUCCESS);
        configFile.close();
    } else {
    ...
    // Load project assets
    LoadProjectAssets();
    ::Log("Project loaded successfully: " + projectPath, Debug::LogLevel::SUCCESS);
...
void HandlerProject::ScanAssetsFolder(const std::string& rootFolder) {
    ...
    for (const auto& entry : fs::recursive_directory_iterator(rootFolder))
    {
        if (entry.is_regular_file())
        {
            ...
            ::Log("File Path: "+filePath+" Parent Folder: "+parentFolder+" File Name: "+filename, Debug::LogLevel::INFO);
    ...
    if (ImGui::InvisibleButton("##ItemButton", totalSize)) {
        if (isDoubleClick) {
            ...
            if (node.isDirectory) {
                ...
                ::Log("Current Directory changed to: " + currentDirectory, Debug::LogLevel::SUCCESS);
            ...
            Log("Time From Struct AssetFile: "+to_string(node.lastClickTime) +" Time Now: "+to_string(currentTime), LogLevel::SUCCESS);
            selectedAsset = nullptr;
            localPath = node.fullPath;
            ::Log("Current Directory changed to: " + localPath, Debug::LogLevel::SUCCESS);
...
bool HandlerProject::CheckForFileChanges() {
    ...
    try {
        ...
        ::Log("Error in CheckForFileChanges: " + std::string(e.what()), Debug::LogLevel::CRASH);
    }
