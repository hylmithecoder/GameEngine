#include "../../../include/ui/VideoPlayer.hpp"
#include <iostream>
using namespace std;
// #include <swresample.h>

VideoPlayer::VideoPlayer() : audioDeviceID(0), swrContext(nullptr) {}
...
bool VideoPlayer::openAudio() {
  ...
  const AVCodec *audioCodec = avcodec_find_decoder(
      formatContext->streams[audioStream]->codecpar->codec_id);
  if (!audioCodec) {
  ...
  if (avcodec_parameters_to_context(
          audioCodecContext, formatContext->streams[audioStream]->codecpar) <
      0) {
  ...
  av_opt_set_int(swrContext, "in_channel_layout",
                 audioCodecContext->ch_layout.u.mask, 0);
  ...
  if (swr_init(swrContext) < 0) {
    std::cerr << "[Video Player] Failed to initialize resampling context: "
              << swrContext << endl;
    return false;
  }
  cout << "Resampling context initialized: " << swrContext << endl;

  // 7. Siapkan SDL Audio Device Stream
  SDL_AudioSpec want;
  want.format = SDL_AUDIO_S16;
  want.channels = audioCodecContext->ch_layout.nb_channels;
  want.freq = audioCodecContext->sample_rate;

  sdlAudioStream = SDL_OpenAudioDeviceStream(SDL_AUDIO_DEVICE_DEFAULT_PLAYBACK,
                                             &want, nullptr, nullptr);
  if (!sdlAudioStream) {
    std::cerr << "Failed to open audio device stream: " << SDL_GetError()
              << std::endl;
    return false;
  }

  // Mulai pemutaran audio
  SDL_ResumeAudioDevice(SDL_GetAudioStreamDevice(sdlAudioStream));

  return true;
}

SDL_Texture *VideoPlayer::convertFrameToTexture(AVFrame *frame,
