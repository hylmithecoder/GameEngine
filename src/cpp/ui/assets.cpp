...
bool Assets::LoadTextureFromFile(const char *filename,
  ...
  // Texture creation will be handled by Vulkan later

  // Store texture info and free image memory
  out_texture->TextureID = 0;
...
Color Assets::GetDominantColor(unsigned char *data, int width, int height,
                               int channels) {
  ...
  for (int y = 0; y < height; y++) {
    for (int x = 0; x < width; x++) {
      ...
      if (x < borderSize || x > width - borderSize || y < borderSize ||
          y > height - borderSize) {
        ...
        if (brightness < 0.15f || brightness > 0.95f)
          continue;
  ...
  Log("Edge Dominant Color - R: " + std::to_string(result.r) + ", G: " +
          std::to_string(result.g) + ", B: " + std::to_string(result.b),
      Debug::LogLevel::INFO);
