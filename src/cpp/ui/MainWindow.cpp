...
bool MainWindow::init(const char* title) {
    ...
    ::Log("Main Window Successfully Initialized");
...
bool MainWindow::openVideo(const char* filePath) {
    ...
    // }
    // else {
    //     ::Log("SDL texture created: "+to_string(reinterpret_cast<uintptr_t>(videoPlayer->texture)), Debug::LogLevel::SUCCESS);
    // }
    ...
    if (videoPlayer->isPlaying) {     
        ...
        if (!paused) {
            if (isOnlyRender == true && isOnlyAd == false) {
                // updateVideoFrame();
                ::Log("Only Render Image", Debug::LogLevel::WARNING);
                updateVideoFrameWithOpenGL();
            }
            else if (isOnlyRender == false && isOnlyAd == true) {
                ::Log("Only Render audio", Debug::LogLevel::WARNING);
                updateAudio();
            }
            else {
                if (videoPlayer->fps < 59) {
                    ...
                    // updateMedia();
                    ::Log("Both Render up 60 fps", Debug::LogLevel::WARNING);
