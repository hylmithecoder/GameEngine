...
public:
  IlmeeHub() {
    ...
    projects.push_back(
        {"Awesome Game", "E:/Game Engine Folder/My First Project", "v1.2.3",
         "2 days ago", "Epic adventure game with stunning graphics", true, 0,
         ImVec4(0.8f, 0.2f, 0.2f, 1.0f)});
    projects.push_back({"Data Visualizer", "/home/user/projects/data_viz",
                        "v0.8.1", "1 week ago",
                        "Interactive data visualization tool", false, 1,
                        ImVec4(0.2f, 0.8f, 0.2f, 1.0f)});
    projects.push_back({"Audio Engine", "/home/user/projects/audio_engine",
                        "v2.1.0", "3 days ago",
                        "High-performance audio processing library", true, 3,
                        ImVec4(0.8f, 0.6f, 0.2f, 1.0f)});
    projects.push_back({"Image Editor", "/home/user/projects/img_editor",
                        "v1.0.0", "5 days ago",
                        "Lightweight image editing application", false, 2,
                        ImVec4(0.6f, 0.2f, 0.8f, 1.0f)});
    projects.push_back({"Network Library", "/home/user/projects/netlib",
                        "v3.0.2", "1 month ago",
                        "Cross-platform networking library", false, 3,
                        ImVec4(0.2f, 0.6f, 0.8f, 1.0f)});

    recentFiles = {"config.ini", "shader.glsl", "texture.png", "audio.wav",
                   "script.lua"};
  }
  ...
  void DrawTopBar() {
    ...
    ImGui::BeginChild("TopBar", ImVec2(0, 80), true,
    ...
    if (ImGui::InputTextWithHint("##search", "Search projects...", searchBuffer,
                                 sizeof(searchBuffer))) {
  ...
  void DrawProjectCard(const Project &project, int index) {
    ...
    ImGui::TextColored(ImVec4(0.7f, 0.7f, 0.7f, 1.0f), "v%s",
                       project.version.c_str());
    ImGui::SameLine();
    ImGui::TextColored(ImVec4(0.6f, 0.6f, 0.6f, 1.0f), "• %s",
                       project.lastModified.c_str());

    // Description
    ImGui::TextWrapped("%s", project.description.c_str());
    // Debug::Log("Project Path: "+project.path);
    // Action buttons
    ImGui::PushStyleColor(ImGuiCol_Button, project.accentColor);
    if (ImGui::Button("Open", ImVec2(60, 25))) {
      ...
      if (count != -1) {
        currentPath =
            std::filesystem::path(std::string(exePath, count)).parent_path();
        Debug::Log("Current executable path: " + currentPath.string(),
                   Debug::LogLevel::INFO);
      } else {
        Debug::Log("Failed to get executable path", Debug::LogLevel::CRASH);
        return;
      }

      // Construct editor path
      std::filesystem::path editorPath = currentPath / "GameEngineSDL";
      Debug::Log("Editor Path: " + editorPath.string());

      // Command line argument
      std::string cmdLine = editorPath.string();
      Debug::Log("Command line: " + cmdLine);
      ...
      if (pid == 0) {
        ...
        // Parent process
        Debug::Log("Launched editor with project: " + project.path);
      } else {
        // Fork failed
        std::string errorMsg = "Failed to launch editor. Fork error.";
        Debug::Log(errorMsg, Debug::LogLevel::CRASH);
      }
    }
    ...
    if (ImGui::BeginPopup("ProjectMenu")) {
      ...
      if (ImGui::MenuItem("Delete Project", nullptr, false, false)) {
      } // Disabled for safety
      ImGui::EndPopup();
    }
  ...
  void DrawProjectGrid() {
    ...
    for (int i = 0; i < projects.size(); i++) {
      if (strlen(searchBuffer) > 0) {
        ...
        std::transform(projectName.begin(), projectName.end(),
                       projectName.begin(), ::tolower);
  ...
  void DrawProjectList() {
    ...
    // Table headers
    if (ImGui::BeginTable("ProjectTable", 5,
      ...
      for (int i = 0; i < projects.size(); i++) {
        ...
        ImGui::TableNextColumn();
        const char *typeNames[] = {"Game", "App", "Tool", "Lib"};
        ImGui::TextColored(projects[i].accentColor, "%s",
                           typeNames[projects[i].projectType]);
  ...
  void DrawCreateProjectDialog() {
    ...
    if (ImGui::BeginPopupModal("Create New Project", &showCreateDialog)) {
      ...
      ImGui::InputTextWithHint("Project Name", "Enter project name...",
                               newProjectName, sizeof(newProjectName));
      ImGui::InputTextWithHint("Project Path", "Choose project location...",
                               newProjectPath, sizeof(newProjectPath));

      ImGui::Text("Project Type:");
      ImGui::RadioButton("Game", &newProjectType, 0);
      ImGui::SameLine();
      ImGui::RadioButton("Application", &newProjectType, 1);
      ImGui::SameLine();
      ImGui::RadioButton("Tool", &newProjectType, 2);
      ImGui::SameLine();
      ImGui::RadioButton("Library", &newProjectType, 3);

      ImGui::InputTextMultiline("Description", newProjectDesc,
                                sizeof(newProjectDesc), ImVec2(-1, 100));
      ...
      if (ImGui::Button("Create Project", ImVec2(120, 0))) {
        ...
        newProject.accentColor =
            ImVec4(0.3f + (rand() % 5) * 0.1f, 0.3f + (rand() % 5) * 0.1f,
                   0.3f + (rand() % 5) * 0.1f, 1.0f);
  ...
  void Render() {
    ...
    if (showAbout) {
      ...
      if (ImGui::BeginPopupModal("About Ilmee Hub", &showAbout,
...
public:
  bool Initialize() {
    ...
    window = SDL_CreateWindow("Ilmee Hub", 1000, 580, SDL_WINDOW_RESIZABLE);
    if (!window)
      return false;
