...
void SceneRenderer2D::InitShaders() {
    ...
    ::Log("[InitShaders] Shader Program: " + std::to_string(shaderProgram) + " Gizmo Shader Program: " + std::to_string(gizmoShaderProgram) + " Grid Shader Program: " + std::to_string(gridShaderProgram), Debug::LogLevel::SUCCESS);
}
...
void SceneRenderer2D::checkProgramLinking(GLuint program) {
    ...
    ::Log("Program " + std::to_string(program) + " linked successfully", Debug::LogLevel::SUCCESS);
}
...
void SceneRenderer2D::Init() {
    ...
    {
        ::Log("[Check Again] Grid Shader Program: " + std::to_string(gridShaderProgram), Debug::LogLevel::WARNING);
    }
    ::Log("[Check Again] Grid Shader Program: " + std::to_string(gridShaderProgram), Debug::LogLevel::WARNING);
...
void SceneRenderer2D::RenderSceneToTexture(const Scene& scene) {
    ...
    // ::Log("[Debug] camZoom: " + std::to_string(cameraZoom)
...
void SceneRenderer2D::DrawGrid(const glm::mat4& projection, const glm::mat4& view) {
        ...
        if (!checkShaderUniforms(gridShaderProgram)) {
            ::Log("Missing required uniforms in grid shader!", Debug::LogLevel::CRASH);
            return;
        }
        ...
        if (ImGui::IsItemHovered()) {
            ...
            if (ImGui::GetIO().MouseWheel != 0.0f) {
                ...
                // Clamp zoom
                zoom = std::max(1.0f, std::min(zoom, 10.0f));
...
bool SceneRenderer2D::checkShaderUniforms(GLuint program) {
    // ::Log("Checking shader uniforms for program: " + std::to_string(program), Debug::LogLevel::INFO);
    ...
    for (const auto& uniform : uniforms) {
        try {
            ...
            ::Log(e.what(), Debug::LogLevel::CRASH);
            allValid = false;
        }
    }
...
void SceneRenderer2D::validateUniformLocation(GLuint program, const char* uniformName, const char* uniformType) {
    ...
    // ::Log("Found uniform '" + std::string(uniformName) + 
...
GLuint SceneRenderer2D::CreateShaderProgram(const std::string& vertPath, const std::string& fragPath) {
    ...
    glDeleteShader(vertShader);
    glDeleteShader(fragShader);
    ::Log("[Debug] Shader Program Successfully Created: " + std::to_string(program), Debug::LogLevel::SUCCESS);
    return program;
}
