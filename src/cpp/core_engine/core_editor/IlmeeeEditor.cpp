...
Project::Project(const std::string &name, const std::string &path,
                 ...
                 const std::string &description, bool isFavorite,
    ...
    : name(name), path(path), version(version), lastModified(lastModified),
      ...
      projectType(projectType), accentColor(accentColor) {}
...
void LogInfo(const std::string &message) { Debugger::LogInfo(message); }

void LogWarning(const std::string &message) { Debugger::LogWarning(message); }

void LogError(const std::string &message) { Debugger::LogError(message); }

void LogSuccess(const std::string &message) { Debugger::LogSuccess(message); }

void RecommendationSystem::RecordProjectOpen(const std::string &path,
                                             int projectType) {
  recentlyOpened[path]++;
...
std::vector<RecommendationSystem::Recommendation>
RecommendationSystem::GetRecommendations(
    const std::vector<std::unique_ptr<Project>> &allProjects) {
...
Editor::Editor() { LogInfo("Editor instance created"); }

Editor::~Editor() { LogInfo("Editor instance destroyed"); }
...
void Editor::CreateProject(const std::string &name, const std::string &path) {
  ...
  auto project = std::make_unique<Project>(
      name, path, "1.0.0", ss.str(), "New project created with IlmeeeEditor",
...
void Editor::OpenProject(const std::string &path) {
  ...
  auto project = std::make_unique<Project>(
      projectName, path, "1.0.0", "2024-01-15 10:30:00", "Loaded project",
      false, 1, ImVec4(0.8f, 0.2f, 0.4f, 1.0f));
...
void RecommendationWindow::RefreshRecommendations() {
  if (!recommendationSystem || !projectsPtr)
    return;

  recommendations = recommendationSystem->GetRecommendations(*projectsPtr);
  LogInfo("Recommendations refreshed, count: " +
          std::to_string(recommendations.size()));
}
