...
void Viewport3D::DrawKtxTexture() {
  ...
  static bool render3D =
      true; // Toggle between 3D pipeline render and simple 2D
  ...
  if (ktxTexture == VK_NULL_HANDLE && textureHandler.isTextureLoaded()) {
    ktxTexture = ImGui_ImplVulkan_AddTexture(
        textureHandler.getTextureSampler(), textureHandler.getTextureView(),
  ...
  if (ktxTexture != VK_NULL_HANDLE) {
    ...
    textureHandler.camera.setRotation(
        glm::vec3(rotation[0], rotation[1], rotation[2]));
    textureHandler.camera.setPosition(
...
void Viewport3D::DrawViewport3D() {
  ...
  static float currentPosition[3] = {0.0f, 0.0f, -2.5f},
               currentRotation[3] = {0.0f, 0.0f, -180.0f};
  ...
  if (viewportTexture == VK_NULL_HANDLE) {
    viewportTexture =
        ImGui_ImplVulkan_AddTexture(offscreenSampler, offscreenImageView,
                                    VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
  }
  ...
  position = glm::vec4(currentPosition[0], currentPosition[1],
                       currentPosition[2], 0.0f);
  rotation =
      glm::vec3(currentRotation[0], currentRotation[1], currentRotation[2]);
  camera.setPosition(position);
  camera.setRotation(rotation);
  // showCurrentCameraPosition();
  Text("Camera Rotation: (%.2f, %.2f, %.2f)", camera.rotation.x,
  ...
  if (viewportWidth != static_cast<uint32_t>(newWidth) ||
      viewportHeight != static_cast<uint32_t>(newHeight)) {
    viewportWidth = static_cast<uint32_t>(newWidth);
    viewportHeight = static_cast<uint32_t>(newHeight);

    // Ensure we recreate the framebuffer and render pass components
    recreateOffscreenResources(viewportWidth, viewportHeight);

    // Update the ImGui texture descriptor to point to the newly created image
    // view
    if (viewportTexture != VK_NULL_HANDLE) {
...
void Viewport3D::HandleCameraMovement() {
  ...
  const Uint8 *state =
      reinterpret_cast<const Uint8 *>(SDL_GetKeyboardState(NULL));
...
void Viewport3D::videoPlayerUI() {
  static char videoPath[5000] = "/home/hylmi/GameEngineFolder/My First "
                                "Project/assets/videos/Usagi Flap.mp4";
  ...
  if (imageHandler.isPlaying) {
    ...
    SameLine();
    if (Button("Stop")) {
      av_seek_frame(imageHandler.formatContext, imageHandler.videoStream, 0,
    ...
    // Informasi video
    Text("Resolution: %dx%d | FPS: %.2f", imageHandler.width,
         imageHandler.height, imageHandler.fps);
    if (!isOnlyRender | isOnlyAd) {
      Separator();
      Text("Frequency: %d | Channels: %d",
           imageHandler.audioCodecContext->sample_rate,
           imageHandler.audioCodecContext->ch_layout.nb_channels);
    }
  } else {
...
void Viewport3D::DrawImage() {
  ...
  if (imguiTexture == VK_NULL_HANDLE) {
    imguiTexture =
        imageHandler.LoadImage("assets/images/backgrounds/Hanako_Swimsuit.png");
  }
  Begin("Image Test");
